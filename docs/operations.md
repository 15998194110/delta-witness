...
The Worker writes privacy-conscious aggregates to the `delta_witness_events` Analytics Engine dataset. It stores normalized channel/partner labels, event type, route, success state, prices, Browser milliseconds, artifact bytes, estimated variable cost, and estimated contribution margin. It does not write target URLs, payment signatures, IP addresses, or user agents.
