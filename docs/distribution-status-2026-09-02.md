...
|---|---|---|---|---|---|---|---:|---:|---:|---|---|
| Agent402 | Accepted: 5 tools, 2 paid, Base mainnet, routable | Fresh crawl at 2026-09-02 15:43 UTC indexed the buyer-intent slugs. All three target queries now recall DELTA in the top 25: `browser verification`, `page state proof`, and `preflight autonomous action`. Smart Order Router dispatch remains `settlement_required` until the Treasury has enough independent settlement history. | Agent402 crawler requested both routes | Both routes returned x402 v2 402, 30,000 atomic Base USDC, Treasury payTo | none | no | $0 | $0 fulfillment cost | $0 | [seller detail](https://agent402.tools/api/index?seller=delta-witness-api.ruphussten.workers.dev) | none |
