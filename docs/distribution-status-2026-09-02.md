...
|---|---|---|---|---|---|---|---:|---:|---:|---|---|
| Agent402 | Accepted: 5 tools, 2 paid, Base mainnet, routable | Seller lookup finds Capture and Guard; pre-change queries `preserve public page` rank Capture #4 and `observe public source deterministic baseline` rank Guard #1. Three target phrases require a metadata redeploy/reindex. | Agent402 crawler requested both routes | Both routes returned x402 v2 402, 30,000 atomic Base USDC, Treasury payTo | none | no | $0 | $0 fulfillment cost | $0 | [seller detail](https://agent402.tools/api/index?seller=delta-witness-api.ruphussten.workers.dev) | none |
